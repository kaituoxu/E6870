...
//  $Id: lab3_lm.C,v 1.28 2009/10/16 02:07:56 stanchen Exp $
...
Lab3LmMain::Lab3LmMain(const map<string, string>& params,
    const SymbolTable& symTable, int n, int bosIdx, int eosIdx, int unkIdx) :
    m_params(params), m_symTable(symTable), m_n(n), m_bosIdx(bosIdx),
    m_eosIdx(eosIdx), m_unkIdx(unkIdx),
    ...
    m_posIdx(0), m_totWordCnt(0), m_totLogProb(0.0), m_sentLogProb(0.0)
    {
    if (!m_wordProbFile.empty())
        m_wordProbStrm.open(m_wordProbFile.c_str());
    if (!m_sentProbFile.empty())
        m_sentProbStrm.open(m_sentProbFile.c_str());
    }

bool Lab3LmMain::init_utt()
    {
    if (m_inStrm.peek() == EOF)
        return false;
    ...
    convert_words_to_indices(m_wordList, m_wordIdxList, m_symTable, m_n,
        m_bosIdx, m_eosIdx, m_unkIdx);
    m_posIdx = m_n - 1;
    m_sentLogProb = 0.0;
    return true;
    }
...
bool Lab3LmMain::init_word()
    {
    if (m_posIdx >= (int) m_wordIdxList.size())
        return false;
    m_ngramBuf.clear();
...
void Lab3LmMain::finish()
    ...
    m_inStrm.close();
    if (!m_wordProbFile.empty())
        m_wordProbStrm.close();
    if (!m_sentProbFile.empty())
        m_sentProbStrm.close();
    cout << format("%.4f PP (%d words)") % exp(m_totWordCnt ?
        -m_totLogProb / m_totWordCnt : 0.0) % m_totWordCnt << endl;
    }
