...
double forward_backward(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    // set the start state to be 0
    int start = graph.get_start_state();//start = 1
    chart(0,start).set_forw_log_prob(0);
    
    //set the remaining rows
    for(int i = 1;i <= frmCnt;i++){
        for(int k = 0;k < stateCnt;k++){
            if(chart(i - 1,k).get_forw_log_prob() > g_zeroLogProb){
                int stateIdx = k;
                // Get number of outgoing arcs for state stateIdx.
                int arcCnt = graph.get_arc_count(stateIdx);
                // Get arc ID of first outgoing arc of state stateIdx.
                ...
                for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                    ...
                    int gmm = arc.get_gmm();
                    double gmmprob = gmmProbs(i - 1, gmm);
                    double prob = chart(i - 1,k).get_forw_log_prob() + arc.get_log_prob() + gmmprob;
                    vector<double> logps;
                    logps.push_back(chart(i, arc.get_dst_state()).get_forw_log_prob());
                    logps.push_back(prob);
                    double logProb = add_log_probs(logps);
                    logps.clear();
                    chart(i, arc.get_dst_state()).set_forw_log_prob(logProb);
                }
            }
        }
    }
    ...
    graph.get_final_state_list(finalstate);
    int states = finalstate.size();
    for(int j = 0;j < states;j++){
        chart(frmCnt,finalstate[j]).set_back_log_prob(0);
    }
    
    //set the remaining rows
    for(int i = frmCnt - 1;i >= 0;i--){
        for(int k = 0;k < stateCnt;k++){
            int stateIdx = k;
            // Get number of outgoing arcs for state stateIdx.
            int arcCnt = graph.get_arc_count(stateIdx);
            ...
            for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                ...
                int preId = arcId;
                arcId = graph.get_arc(arcId, arc);
                if(chart(i + 1, arc.get_dst_state()).get_back_log_prob() > g_zeroLogProb){
                    int gmm = arc.get_gmm();
                    double gmmprob = gmmProbs(i, gmm);
                    double prob = chart(i + 1, arc.get_dst_state()).get_back_log_prob() + arc.get_log_prob() + gmmprob;
                    vector<double> logps;
                    logps.push_back(chart(i, graph.get_src_state(preId)).get_back_log_prob());
                    logps.push_back(prob);
                    double logProb = add_log_probs(logps);
                    logps.clear();
                    chart(i, graph.get_src_state(preId)).set_back_log_prob(logProb);
                }
            }
        }
    }
         for(int i = 0 ;i < frmCnt;i++){
            for(int k = 0;k < stateCnt;k++){
                if(chart(i,k).get_forw_log_prob() > g_zeroLogProb){
                    int stateIdx = k;
                    // Get number of outgoing arcs for state stateIdx.
                    int arcCnt = graph.get_arc_count(stateIdx);
                    // Get arc ID of first outgoing arc of state stateIdx.
                    ...
                    for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                        ...
                        //  to arc ID of the next outgoing arc.
                        arcId = graph.get_arc(arcId, arc);
                        double forwd = chart(i, k).get_forw_log_prob();
                        double backwd = chart(i + 1, arc.get_dst_state()).get_back_log_prob();
                        double arcprob = arc.get_log_prob();
			double gmmprob = gmmProbs(i, arc.get_gmm());
                        double arcPosterior = forwd + backwd + arcprob - uttLogProb + gmmprob;
                        arcPosterior = exp(arcPosterior);
