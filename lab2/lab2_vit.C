...
double viterbi(const Graph& graph, const matrix<double>& gmmProbs,
    ...
    //cout << "frmCnt:" << frmCnt << endl;
    //cout << "stateCnt:" << stateCnt << endl;
    //int GMMs = gmmProbs.size2();
    //cout << "GMMs:" << GMMs << endl;
    //cout << "gmmProbsnum:" << gmmProbs(2,3) << endl;
    
    // set the start state to be (0, -1)
    int start = graph.get_start_state();//start = 1
    chart(0,start).assign(0,-1);
       
    //set the remaining rows
    for(int i = 1;i <= frmCnt;i++){
        for(int k = 0;k < stateCnt;k++){
            if(chart(i - 1,k).get_log_prob() > g_zeroLogProb){
                int stateIdx = k;
                ...
                for (int arcIdx = 0; arcIdx < arcCnt; ++arcIdx){
                    ...
                    double prob = chart(i - 1,k).get_log_prob() + arc.get_log_prob() + gmmprob;
                    if(prob > chart(i, arc.get_dst_state()).get_log_prob()){
                        chart(i, arc.get_dst_state()).assign(prob, pre_id);
                    }
                }
            }
         }
