...
void FrontEnd::do_window(const matrix<double>& inFeats,
    ...
    for(int i = 0;i < outFrameCnt;i++){
        for(int j = 0;j < sampPerWindow;j++){
	  outFeats(i,j) = inFeats(j + sampShift*i,0);
        }
    }
    for(int i = 0;i < outFrameCnt;i++){
        for(int j = 0;j < sampPerWindow;j++){
	  double a = 2*M_PI*j;
	  double mul = 0.54 - 0.46*cos(a/(sampPerWindow - 1));
	  outFeats(i,j) = mul * outFeats(i,j);
        }
    }
...
void FrontEnd::do_melbin(const matrix<double>& inFeats,
    ...
    double fmax = 1 / (2 * samplePeriod);
    double melfmax = 1127 * log(1 + fmax/700);
    double interv = melfmax / (numBins + 1);
    //cout << format("melfmax %ls\n") % melfmax;
    //cout << format("interv %ls\n") % interv;
    matrix<double> fre;
    fre.resize(numBins + 2, 1);
    for(int m = 0;m < numBins + 2;m++){
         fre(m,0) = interv * m;
    }
    for(int idx = 0;idx < inFrameCnt;idx++){
      for(int i = 0;i < numBins;i++){
	    double sum = 0.0;
            for(int j = 0;j < inDimCnt;j+=2){
	      double f = j / (2 * inDimCnt * samplePeriod);
	      double melf = 1127 * log(1 + f/700);
	      double x = sqrt(pow(inFeats(idx,j),2) + pow(inFeats(idx,j + 1),2));
	      double h = 0.0;
	      if(melf >= fre(i,0) && melf <= fre(i + 1,0)){
		h = (melf - fre(i,0))/(fre(i + 1,0) - fre(i,0));
	      }
	      else if(melf >= fre(i + 1,0) && melf <= fre(i + 2,0)){
		h = (fre(i + 2,0) - melf)/(fre(i + 2,0) - fre(i + 1,0));
	      }
	      else {h = 0;}
	      sum = sum + x * h;
            }outFeats(idx,i) = sum;
...
void FrontEnd::do_dct(const matrix<double>& inFeats,
    ...
    for(int i = 0;i < numCoeffs;i++){
      for(int idx = 0;idx < inFrameCnt;idx++){
        double sum = 0;
        for(int j = 0;j < inDimCnt;j++){
	  double temp = inFeats(idx,j)*cos(M_PI*(i + 1)*(j + 0.5)/inDimCnt);
          sum = sum + temp;
        }
        outFeats(idx,i) = sqrt(2.0/inDimCnt)*sum;
      }
    }
